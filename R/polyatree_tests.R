pt_ci_test <- function(X, Y, Z = NULL, rho = 0.5, c = 1,
                       max_depth = -1, qdist = qnorm, aj = function(depth) depth^2,
                       verbose = TRUE, log_BF = FALSE) {
  if (is.null(Z)) {
    if (verbose) cat('Redirecting to independence test\n')
    return(pt_independence_test(X, Y, c, max_depth, qdist, aj, verbose, log_BF))
  }

  if (.is_discrete(X) || .is_discrete(Y)) {
    if (verbose) cat('Performing conditional two-sample test\n')
    return(pt_d_sample_ci_test(X, Y, Z, rho, c, max_depth, qdist, aj, log_BF))
  }

  if (verbose) cat('Performing continuous conditional independence test\n')
  return(pt_continuous_ci_test(X, Y, Z, rho, c, max_depth, qdist, aj, log_BF))
}

pt_d_sample_ci_test <- function(X, Y, Z, rho = 0.5, c = 1,
                                max_depth = -1, qdist = qnorm, aj = function(depth) depth^2, log_BF = FALSE) {
  ...
  p_H0 <- .condopt_marginal_likelihood(XYZ, target_idx = 1, z_idx = 3,
                                       z_min = 0, z_max = 1,
                                       c = c, rho = rho, depth = 1, max_depth, qdist, aj)
...
pt_continuous_ci_test <- function(X, Y, Z = NULL, rho = 0.5, c = 1,
  ...
  phi_x <- .condopt_marginal_likelihood(XYZ, target_idx = 1, z_idx = 3,
                                        z_min = 0, z_max = 1,
                                        c = 2 * c, rho = rho, depth = 1, max_depth, qdist, aj)
  ...
  phi_xy_dep <- .condopt_marginal_likelihood(XYZ, target_idx = c(1, 2), z_idx = 3,
                                             z_min = 0, z_max = 1,
                                             c = 1 * c, rho = rho, depth = 1, max_depth, qdist, aj)
...
.condopt_marginal_likelihood <- function(data, target_idx, z_idx, z_min, z_max,
                                         c, rho, depth, max_depth, qdist, aj) {
  ...
  logl <- .pt_marginal_likelihood(localData, low = rep(0, ncol(localData)),
                                         ...
                                         c = c, depth = 1, max_depth, qdist, aj)
  ...
  phi_1 <- .condopt_marginal_likelihood(data, target_idx, z_idx, z_min, (z_min + z_max) / 2,
                                        c, rho, depth + 1, max_depth, qdist, aj)
...
pt_independence_test <- function(X, Y, c = 1, max_depth = -1, qdist = qnorm, aj = function(depth) depth^2, verbose = TRUE, log_BF = FALSE) {
  if (.is_discrete(X) || .is_discrete(Y)) {
    if (verbose)
      cat('Performing two-sample test\n')
    return(pt_d_sample_test(X, Y, c = c, max_depth = max_depth, qdist = qdist, aj, log_BF))
  }

  if (verbose)
    cat('Performing continuous independence test\n')
  return(pt_continuous_independence_test(X, Y, c = c, max_depth = max_depth, qdist = qdist, aj, log_BF))
}
...
pt_continuous_independence_test <- function(X, Y, c = 1, max_depth = -1, qdist = qnorm, aj = function(depth) depth^2, log_BF = FALSE) {
  old_expressions <- options()$expressions
  options(expressions = max(max_depth, old_expressions))
