...
mean_shift_paper <- function(base, C) {
  theta <- sample(c(2, 3, 4, 5, 6), 1)
  (1 - C) * base + C * (base + theta)
}

variance_shift_paper <- function(base, C) {
  theta <- sample(c(2, 3, 4, 5, 6), 1)
  (1 - C) * base + C * (1 + theta) * base
}

fixed_point_paper <- function(base, C) {
  theta <- sample(c(2, 3, 4, 5, 6), 1)
  (1 - C) * base + C * theta
}

mixture_paper <- function(base, C) {
  theta <- sample(c(2, 3, 4, 5, 6), 1)
  idx <- sample(c(-1, theta), length(C), replace = TRUE)
  (1 - C) * base + C * (base + idx)
}

linear_paper <- function(X) {
  X
}


parabolic_paper <- function(X) {
  X ^ 2
}

sinusoidal_paper <- function(X) {
